...
export function SkillCard({ icon: Icon, title, children }: SkillCardProps) {
  return (
    <Card placeholder="placeholder" color="transparent" shadow={false}>
      <CardBody placeholder="placeholder" className="grid justify-center text-center">
        <div className="mx-auto mb-6 grid h-12 w-12 place-items-center rounded-full bg-gray-900 p-2.5 text-white shadow">
          <Icon className="h-6 w-6" strokeWidth={2} />
        </div>
        <Typography placeholder="placeholder" color="blue-gray" className="mb-2">
          {title}
        </Typography>
        <Typography placeholder="placeholder" className="px-8 font-normal !text-gray-500">
          {children}
        </Typography>
      </CardBody>
    </Card>
